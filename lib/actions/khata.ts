...
export async function deleteKhataTransaction(id: string) {
  try {
    ...
    // Safety: manual payment (debit) cancellation can desync invoice settlement
    // because per-invoice allocation isn't persisted on the transaction.
    // Until allocation mapping is stored, block debit cancellation.
    if (transaction.type === 'debit') {
      return { error: "Cannot cancel payment entries automatically. Add a reversing credit/debit entry instead." };
    }
